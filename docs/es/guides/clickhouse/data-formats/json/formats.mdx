...
Una de las formas más habituales de los datos JSON consiste en tener una lista de objetos JSON dentro de un array JSON, como en [este ejemplo](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json):
...
Para importar una lista de objetos JSON, podemos usar el formato [`JSONEachRow`](/es/reference/formats/JSON/JSONEachRow) (insertando datos del archivo [list.json](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)):
...
En algunos casos, la lista de objetos JSON puede codificarse como propiedades del objeto en lugar de elementos del array (consulta [objects.json](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json) para ver un ejemplo):
...
A veces, para ahorrar espacio, los archivos JSON se codifican como arrays en lugar de objetos. En este caso, trabajamos con una [lista de arrays JSON](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.jsonl):
...
En algunos casos, los datos pueden codificarse por columnas en lugar de por filas. En este caso, un objeto JSON principal contiene columnas con valores. Echa un vistazo al [siguiente archivo](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json):
...
También se admite un formato más compacto cuando se trabaja con un [array de columnas](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json) en lugar de un objeto, con el formato [`JSONCompactColumns`](/es/reference/formats/JSON/JSONCompactColumns):
...
En algunos casos, puede que quiera guardar objetos JSON en una sola columna `String` (o `JSON`) en lugar de analizarlos. Esto puede ser útil al trabajar con una lista de objetos JSON con estructuras diferentes. Tomemos [este archivo](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json) como ejemplo, donde tenemos varios objetos JSON distintos dentro de una lista principal:
...
En los casos en los que trabajamos con [objetos JSON anidados](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json), también podemos definir un esquema explícito y usar tipos complejos ([`Array`](/es/reference/data-types/array), [`JSON`](/es/guides/clickhouse/data-formats/json/intro) o [`Tuple`](/es/reference/data-types/tuple)) para cargar los datos:
...
Podemos hacer referencia a [claves JSON anidadas](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json) habilitando la [siguiente opción de configuración](/es/reference/settings/formats#input_format_import_nested_json):
...
Todavía podemos insertar los [datos JSON originales](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) con 3 columnas en esta tabla:
...
Para importar datos BSON, usamos el formato [BSONEachRow](/es/reference/formats/BSONEachRow). Importemos los datos desde [este archivo BSON](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson):

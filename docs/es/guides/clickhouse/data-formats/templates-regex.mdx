...
Supongamos que queremos importar datos del siguiente [archivo de log](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/error.log):
...
Para importar datos con una plantilla determinada, tenemos que guardar la cadena de la plantilla en un archivo ([row.template](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/row.template) en nuestro caso):
...
[Plantilla del conjunto de resultados](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.results), que define la estructura del conjunto de resultados completo:
...
Aquí, `rows_read` y `time` son métricas del sistema disponibles para cada petición. Mientras que `data` representa las filas generadas (`${data}` siempre debe aparecer como el primer marcador de posición en este archivo), a partir de una plantilla definida en un [**archivo de plantilla de filas**](/eshttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.rows):

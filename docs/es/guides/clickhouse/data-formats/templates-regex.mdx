...
Supongamos que queremos importar datos del siguiente [archivo de log](/es/assets/error.log):
...
Para importar datos con una plantilla determinada, tenemos que guardar la cadena de la plantilla en un archivo ([row.template](/es/assets/row.template) en nuestro caso):
...
[Plantilla del conjunto de resultados](/es/assets/output.results), que define la estructura del conjunto de resultados completo:
...
Aquí, `rows_read` y `time` son métricas del sistema disponibles para cada petición. Mientras que `data` representa las filas generadas (`${data}` siempre debe aparecer como el primer marcador de posición en este archivo), a partir de una plantilla definida en un [**archivo de plantilla de filas**](/es/assets/output.rows):
...
Los resultados basados en plantillas también se pueden exportar a archivos mediante la cláusula [`INTO OUTFILE`](/es/reference/statements/select/into-outfile). Generemos archivos HTML a partir de los formatos [conjunto de resultados](/es/assets/html.results) y [fila](/es/assets/html.row) proporcionados:

...
Para importar datos del [archivo CSV](/es/assets/data_small.csv) a la tabla `sometable`, podemos pasar el archivo directamente a `clickhouse-client` mediante una tubería:
...
Supongamos que nuestro [archivo CSV incluye encabezados](/es/assets/data_small_headers.csv):
...
El [archivo](/es/assets/data_small.csv) tiene 1k filas, pero ClickHouse solo cargó 990, ya que le indicamos que omitiera las primeras 10.
...
El formato de datos separados por tabuladores se utiliza ampliamente como formato de intercambio de datos. Para cargar datos desde un [archivo TSV](/es/assets/data_small.tsv) en ClickHouse, se utiliza el formato [TabSeparated](/es/reference/formats/TabSeparated/TabSeparated):
...
Este formato incluirá dos filas de encabezado: una con los nombres de las columnas y la otra con los tipos de columna. Esto permitirá que ClickHouse (y otras aplicaciones) identifiquen los tipos de columna al cargar datos desde [estos archivos](/es/assets/data_csv_types.csv):
...
Ahora podemos cargar datos desde nuestro [archivo con formato personalizado](/es/assets/data_small_custom.txt):

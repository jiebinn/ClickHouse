...
Para importar desde un [archivo Avro](/es/assets/data.avro), debemos usar el formato [Avro](/es/reference/formats/Avro/Avro) en la sentencia `INSERT`:
...
Otro formato columnar es [Apache Arrow](https://arrow.apache.org/), que ClickHouse también admite para importar y exportar datos. Para importar datos desde un [archivo Arrow](/es/assets/data.arrow), usamos el formato [Arrow](/es/reference/formats/Arrow/Arrow):
...
El formato [Apache ORC](https://orc.apache.org/) es un formato de almacenamiento columnar que se utiliza habitualmente con Hadoop. ClickHouse admite tanto la importación como la exportación de [datos ORC](/es/assets/data.orc) mediante el [formato ORC](/es/reference/formats/ORC):

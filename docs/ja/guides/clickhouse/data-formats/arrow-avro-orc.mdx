...
[avro file](/ja/assets/data.avro)からインポートするには、`INSERT`ステートメントで[Avro](/ja/reference/formats/Avro/Avro)フォーマットを使用します。
...
もう1つの列指向フォーマットとして、[Apache Arrow](https://arrow.apache.org/) があります。これは ClickHouse でもインポートとエクスポートの両方に対応しています。[Arrowファイル](/ja/assets/data.arrow) からデータをインポートするには、[Arrow](/ja/reference/formats/Arrow/Arrow) フォーマットを使用します。
...
[Apache ORC](https://orc.apache.org/) フォーマットは、通常 Hadoop で使用される列指向ストレージフォーマットです。ClickHouse は、[ORC フォーマット](/ja/reference/formats/ORC) を使用した [ORC データ](/ja/assets/data.orc) のインポートとエクスポートの両方に対応しています。

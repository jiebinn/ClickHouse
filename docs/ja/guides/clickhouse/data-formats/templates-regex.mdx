...
次の[ログファイル](/ja/assets/error.log)からデータをインポートする場合を考えます。
...
指定したTemplateを使ってデータをインポートするには、テンプレート文字列をファイルに保存する必要があります (この例では [row.template](/ja/assets/row.template)) :
...
[結果セットテンプレート](/ja/assets/output.results)：結果セット全体のレイアウトを定義します。
...
ここで、`rows_read` と `time` は各リクエストで利用できるシステムメトリクスです。一方、`data` は生成された行を表します (このファイルでは `${data}` が常に最初のプレースホルダーである必要があります) 。これは、[**行テンプレートファイル**](/ja/assets/output.rows) で定義されたテンプレートに基づくものです。
...
Template ベースの結果は、[`INTO OUTFILE`](/ja/reference/statements/select/into-outfile) 句を使ってファイルにエクスポートすることもできます。指定された[結果セット](/ja/assets/html.results)および[行](/ja/assets/html.row)フォーマットに基づいて、HTML ファイルを生成してみましょう。
...
[Regexp](/ja/reference/formats/Regexp) フォーマットは、入力データをより複雑な形で解析する必要がある、より高度なケースに対応しています。今回は [error.log](/ja/assets/error.log) のサンプルファイルを解析し、ファイル名とプロトコルも抽出して、それぞれ別のカラムに保存してみましょう。まず、そのための新しいテーブルを用意します。

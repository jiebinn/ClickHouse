...
データを読み込む前に、[file()](/ja/reference/functions/regular-functions/files#file) 関数を使って、[サンプルの Parquet ファイル](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.parquet) の構造を確認できます。
...
ClickHouse と Parquet のデータ型はほぼ同じですが、[若干の違いがあります](/ja/reference/formats/Parquet/Parquet#data-types-matching-parquet)。たとえば、ClickHouse は `DateTime` 型を Parquet の `int64` としてエクスポートします。これを再度 ClickHouse にインポートすると、数値として表示されます ([time.parquet file](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/time.parquet)) :

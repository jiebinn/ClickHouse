...
ここではデモ用に some&#95;data の[テーブル](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.sql)と[データ](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.tsv)を使用します。必要に応じて、ご自身の ClickHouse インスタンスで再現してみてください。
...
これにより、[data.clickhouse](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.clickhouse) ファイルがNative フォーマットで作成されます。
...
これにより、[data.binary](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.binary) ファイルがバイナリ行形式で生成されます。
...
[MessagePackファイル](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.msgpk) からデータをインポートするには、
...
[Protocol Buffers](/ja/reference/formats/Protobuf/Protobuf) を扱うには、まず [スキーマファイル](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.proto) を定義します。
...
これにより、データは [proto.bin](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/proto.bin) ファイルに保存されます。ClickHouse では、ネストされたメッセージに加えて、Protobuf データのインポートもサポートしています。単一の Protocol Buffer メッセージを扱う場合は、[ProtobufSingle](/ja/reference/formats/Protobuf/ProtobufSingle) の使用を検討してください (この場合、長さを示す区切り文字は省略されます) 。
...
ClickHouse がサポートする、もう 1 つの一般的なバイナリシリアライゼーションフォーマットとして、[Cap&#39;n Proto](https://capnproto.org/) があります。`Protobuf` 形式と同様に、この例でもスキーマファイル ([`schema.capnp`](/jahttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.capnp)) を定義する必要があります。

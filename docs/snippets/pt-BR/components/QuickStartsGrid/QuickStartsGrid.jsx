export const QuickStartsGrid = ({ quickStartsData = [], featured = [] }) => {
  ...
  const productOptions = [
    ...
    { value: 'language-clients', label: "Cliente por linguagem" },
  ...
  const featuredQuickStarts = featured
    .map(f => {
      ...
      return {
        id: f.id,
        href: (qs && qs.href) || `/pt-BR/get-started/quickstarts/${f.id}`,
        title: (qs && qs.title) || '',
      };
    })
    .filter(f => f.title);
              ...
              {filteredQuickStarts.length > 0 ? (
                ...
                <div className="text-center py-12 flex flex-col items-center">
                  <p className="text-gray-600 dark:text-gray-400 text-lg block">
                    Nenhum guia de início rápido encontrado para os critérios informados.
                  </p>

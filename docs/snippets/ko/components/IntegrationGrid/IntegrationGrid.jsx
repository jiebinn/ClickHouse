export const IntegrationGrid = () => {
  ...
  function getSectionDescription(type) {
    const descriptions = {
      ClickPipes: "ClickPipes는 다양한 소스에서 대용량 데이터를 수집하는 작업을 몇 번의 클릭만으로 간단하게 처리할 수 있는 통합 엔진입니다.",
      "Data ingestion": "ClickHouse로 데이터 파이프라인을 간소화하십시오! 원활한 통합으로 효율적인 데이터 수집을 보장하고 실시간 분석을 최적화합니다.",
      "Data visualization": "데이터로 더 명확한 인사이트를 전달하십시오! ClickHouse 통합은 시각화를 강화하여 인사이트를 더욱 생생하고 실용적으로 만듭니다.",
      "SQL client": "익숙한 SQL 클라이언트 도구와 인터페이스를 사용하여 ClickHouse 데이터베이스에 접근하고 쿼리를 실행하십시오.",
      "Language client": "익숙한 언어로 개발하십시오! ClickHouse의 언어 클라이언트 통합을 통해 다양한 프로그래밍 언어에서 데이터에 원활하게 접근할 수 있습니다.",
      "AI/ML": "통합된 ML 도구와 프레임워크를 활용하여 머신러닝 및 AI 워크로드에 ClickHouse를 사용하십시오.",
      "Data management": "전문 관리 도구를 사용하여 ClickHouse 데이터를 관리, 모니터링 및 최적화하십시오.",
      "Data integration": "기존 데이터 인프라 및 워크플로와 ClickHouse를 통합하십시오.",
      "Security governance": "ClickHouse 환경에 보안 및 거버넌스 프레임워크를 구현하십시오."
    }
    return descriptions[type] || "전문 도구 및 서비스와 ClickHouse를 통합하십시오."
  }

  // 일반 렌더 함수(컴포넌트 아님): IntegrationGrid가 렌더링될 때마다 재마운트되지 않고
  // key를 기준으로 카드를 조정합니다. isDark는 최상위 레벨의 단일
  // useDarkMode() 호출에서 전달됩니다.
  function renderIntegrationCard(integration, isDark, key) {
    ...
    // 외부 링크는 문서 외부(clickhouse.com/docs 제외)를 가리킵니다.
  ...
  function useCMSIntegrations() {
    // Mintlify는 테마 전환 시 전체 콘텐츠 서브트리를 재마운트하므로,
    // "Loading…" 상태가 다시 표시되고 재요청이 발생할 수 있습니다.
    // 카드가 즉시 렌더링되도록 window의 크로스 리마운트 캐시에서 상태를 초기화합니다.
    // effect는 백그라운드에서 계속 갱신됩니다.
      ...
      const fetchIntegrations = async () => {
        ...
        try {
          ...
          const timeoutId = setTimeout(() => {
            controller.abort()
            console.log("CMS 요청이 8초 후 시간 초과되었습니다.")
          }, 8000)
          ...
          cacheIntegrations(transformedData)
          setError(null)
          console.log("최신 CMS 데이터로 성공적으로 업데이트되었습니다.")
        } catch (cmsErr) {
          ...
          if (integrations.length === 0) {
            setError("통합 목록을 불러올 수 없습니다. 페이지를 새로 고침해 주십시오.")
          }
        } finally {
          setLoading(false)
        }
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">통합 목록을 불러오는 중...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600 dark:text-red-400">통합 목록을 불러오지 못했습니다: {error}</p>
        <p className="text-gray-600 dark:text-gray-400">페이지를 새로 고침해 주십시오.</p>
      </div>
    )
  }
  ...
  if (integrations.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">통합 항목이 없습니다.</p>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {filteredIntegrations.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 text-lg">검색 조건에 맞는 통합이 없습니다.</p>
          </div>
        )}
      </div>
    </>

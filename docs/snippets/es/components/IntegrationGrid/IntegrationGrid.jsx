export const IntegrationGrid = () => {
  ...
  function getSectionDescription(type) {
    const descriptions = {
      ClickPipes: "ClickPipes es un motor de integración que hace que la ingestión de grandes volúmenes de datos desde una variedad de fuentes sea tan sencilla como hacer clic en unos pocos botones.",
      "Data ingestion": "¡Optimiza tus canalizaciones de datos con ClickHouse! Las integraciones fluidas garantizan una ingestión eficiente y optimizan el análisis en tiempo real.",
      "Data visualization": "¡Dale vida a tus datos! Las integraciones de ClickHouse potencian la visualización, haciendo que los hallazgos sean más claros y accionables.",
      "SQL client": "Accede y realiza consultas en bases de datos de ClickHouse con las herramientas e interfaces de cliente SQL que ya conoces.",
      "Language client": "¡Programa en tu entorno habitual! Las integraciones de cliente de lenguaje de ClickHouse hacen que el acceso a los datos sea fluido en múltiples lenguajes de programación.",
      "AI/ML": "Aprovecha ClickHouse para cargas de trabajo de machine learning e inteligencia artificial con herramientas y frameworks de ML integrados.",
      "Data management": "Gestiona, supervisa y optimiza tus datos en ClickHouse con herramientas de administración especializadas.",
      "Data integration": "Integra ClickHouse con tu infraestructura de datos y flujos de trabajo existentes.",
      "Security governance": "Implementa marcos de seguridad y gobernanza para tu entorno de ClickHouse."
    }
    return descriptions[type] || "Integra ClickHouse con herramientas y servicios especializados."
  }
      ...
      const fetchIntegrations = async () => {
        ...
        try {
          ...
          const timeoutId = setTimeout(() => {
            controller.abort()
            console.log("La solicitud al CMS superó el tiempo de espera de 8 segundos")
          }, 8000)
          ...
          cacheIntegrations(transformedData)
          setError(null)
          console.log("Actualizado correctamente con datos frescos del CMS")
        } catch (cmsErr) {
          ...
          if (integrations.length === 0) {
            setError("No se pueden cargar las integraciones. Intente actualizar la página.")
          }
        } finally {
          setLoading(false)
        }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600 dark:text-red-400">Error al cargar las integraciones: {error}</p>
        <p className="text-gray-600 dark:text-gray-400">Intente actualizar la página.</p>
      </div>
    )
  }
  ...
  if (integrations.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">No se encontraron integraciones.</p>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {filteredIntegrations.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 text-lg">No se encontraron integraciones que coincidan con sus criterios.</p>
          </div>
        )}
      </div>
    </>

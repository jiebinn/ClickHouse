export const QuickStartsGrid = ({ quickStartsData = [], featured = [] }) => {
  ...
  const useCaseOptions = [
    { value: 'real-time-analytics', label: "Real-time аналитика" },
    { value: 'data-warehousing', label: "Хранилище данных" },
    { value: 'observability', label: "Обсервабилити" },
    { value: 'ai-ml', label: "AI/ML" },
  ];
  const productOptions = [
    { value: 'self-managed', label: 'ClickHouse (с открытым исходным кодом)' },
    { value: 'cloud', label: 'ClickHouse Cloud' },
    { value: 'clickpipes', label: "ClickPipes" },
    { value: 'language-clients', label: "Языковые клиенты" },
  ...
  const featuredQuickStarts = featured
    .map(f => {
      ...
      return {
        id: f.id,
        href: (qs && qs.href) || `/ru/get-started/quickstarts/${f.id}`,
        title: (qs && qs.title) || '',
      };
    })
    .filter(f => f.title);
              ...
              {filteredQuickStarts.length > 0 ? (
                ...
                <div className="text-center py-12 flex flex-col items-center">
                  <p className="text-gray-600 dark:text-gray-400 text-lg block">
                    Не найдено руководств по быстрому старту, соответствующих вашим критериям.
                  </p>

export const IntegrationGrid = () => {
        ...
        >
          ...
          <div className="w-full flex flex-col items-center justify-center gap-2">
            <div className="w-full flex items-center justify-center">
              <img src={getLogoSrc()} alt={`شعار ${integration.integration_title || integration.slug}`} className="object-contain" style={{ width: "64px", height: "64px", pointerEvents: "none" }} />
            </div>
      ...
      const fetchIntegrations = async () => {
        ...
        try {
          ...
          const timeoutId = setTimeout(() => {
            controller.abort()
            console.log("انتهت مهلة طلب CMS بعد 8 ثوانٍ")
          }, 8000)
          ...
          cacheIntegrations(transformedData)
          setError(null)
          console.log("تم التحديث بنجاح ببيانات CMS الحديثة")
        } catch (cmsErr) {
          ...
          if (integrations.length === 0) {
            setError("تعذّر تحميل التكاملات. يرجى تحديث الصفحة والمحاولة مجدداً.")
          }
        } finally {
          setLoading(false)
        }
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">جارٍ تحميل التكاملات...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600 dark:text-red-400">فشل تحميل التكاملات: {error}</p>
        <p className="text-gray-600 dark:text-gray-400">يرجى تحديث الصفحة والمحاولة مجدداً.</p>
      </div>
    )
  }
  ...
  if (integrations.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">لم يتم العثور على أي تكاملات.</p>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {filteredIntegrations.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 text-lg">لا توجد تكاملات تطابق معايير البحث.</p>
          </div>
        )}
      </div>
    </>

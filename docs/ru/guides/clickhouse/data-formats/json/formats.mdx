...
Один из самых распространённых форматов JSON-данных — список объектов JSON в JSON-массиве, как в [этом примере](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json):
...
Чтобы импортировать список объектов JSON, можно использовать формат [`JSONEachRow`](/ru/reference/formats/JSON/JSONEachRow) (для вставки данных из файла [list.json](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)):
...
В некоторых случаях список объектов JSON может быть представлен как свойства объекта, а не как элементы массива (см. пример в [objects.json](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json)):
...
Иногда для экономии места JSON‑файлы кодируют в виде массивов, а не объектов. В этом случае мы имеем дело со [списком JSON-массивов](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.jsonl):
...
В некоторых случаях данные могут быть представлены по столбцам, а не по строкам. В этом случае родительский объект JSON содержит столбцы со значениями. См. [следующий файл](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json):
...
Также поддерживается более компактный формат для работы с [массивом столбцов](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json) вместо объекта с использованием формата [`JSONCompactColumns`](/ru/reference/formats/JSON/JSONCompactColumns):
...
В некоторых случаях объекты JSON удобнее сохранять в одном столбце `String` (или `JSON`), а не разбирать. Это может быть полезно при работе со списком объектов JSON с разной структурой. Возьмем для примера [этот файл](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json), где внутри родительского списка содержится несколько разных объектов JSON:
...
Когда мы имеем дело с [вложенными JSON-объектами](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json), можно дополнительно определить явную схему и использовать сложные типы ([`Array`](/ru/reference/data-types/array), [`JSON`](/ru/guides/clickhouse/data-formats/json/intro) или [`Tuple`](/ru/reference/data-types/tuple)) для загрузки данных:
...
Мы можем обращаться к [ключам вложенных JSON-объектов](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json), включив [следующий параметр настройки](/ru/reference/settings/formats#input_format_import_nested_json):
...
Мы по-прежнему можем вставить в эту таблицу [исходные данные в формате JSON](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) с 3 столбцами:
...
Для импорта данных BSON используется формат [BSONEachRow](/ru/reference/formats/BSONEachRow). Давайте импортируем данные из [этого BSON-файла](/ruhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson):

...
Чтобы импортировать данные из [CSV-файла](/ru/assets/data_small.csv) в таблицу `sometable`, можно направить файл напрямую в `clickhouse-client`:
...
Предположим, что в нашем [CSV-файле есть заголовки](/ru/assets/data_small_headers.csv):
...
В [файле](/ru/assets/data_small.csv) 1k строк, но ClickHouse загрузил только 990, так как мы указали пропустить первые 10.
...
Формат данных с разделителем табуляции широко используется для обмена данными. Чтобы загрузить данные из [файла в формате TSV](/ru/assets/data_small.tsv) в ClickHouse, используется формат [TabSeparated](/ru/reference/formats/TabSeparated/TabSeparated):
...
Этот формат будет содержать две строки заголовка: одну с именами столбцов, другую — с типами столбцов. Это позволит ClickHouse (и другим приложениям) определять типы столбцов при загрузке данных из [таких файлов](/ru/assets/data_csv_types.csv):
...
Теперь мы можем загрузить данные из нашего [файла](/ru/assets/data_small_custom.txt):

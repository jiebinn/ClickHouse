...
Чтобы импортировать данные из [файла Avro](/ru/assets/data.avro), следует использовать формат [Avro](/ru/reference/formats/Avro/Avro) в операторе `INSERT`:
...
Ещё один столбцовый формат — [Apache Arrow](https://arrow.apache.org/), который ClickHouse также поддерживает для импорта и экспорта данных. Чтобы импортировать данные из [файла Arrow](/ru/assets/data.arrow), используется формат [Arrow](/ru/reference/formats/Arrow/Arrow):
...
[Apache ORC](https://orc.apache.org/) — это столбцовый формат хранения данных, обычно используемый в Hadoop. ClickHouse поддерживает как импорт, так и экспорт [данных ORC](/ru/assets/data.orc) с использованием [формата ORC](/ru/reference/formats/ORC):

...
По умолчанию ClickHouse пропускает неизвестные столбцы (это управляется параметром [input&#95;format&#95;skip&#95;unknown&#95;fields](/ru/reference/settings/formats#input_format_skip_unknown_fields)) и обрабатывает данные для первой таблицы, найденной в дампе (если в один файл выгружено несколько таблиц). DDL-операторы будут пропущены. Чтобы загрузить данные из дампа MySQL в таблицу (файл [mysql.sql](/ru/assets/mysql.sql)):

...
Предположим, что мы хотим импортировать данные из следующего [файла журнала](/ru/assets/error.log):
...
Чтобы импортировать данные с помощью указанного шаблона, нужно сохранить строку шаблона в файл ([row.template](/ru/assets/row.template) в нашем случае):
...
[Шаблон результирующего набора](/ru/assets/output.results), который задаёт структуру всего результирующего набора:
...
Здесь `rows_read` и `time` — системные метрики, доступные для каждого запроса. При этом `data` обозначает сгенерированные строки (`${data}` всегда должен идти первым плейсхолдером в этом файле), сформированные по шаблону, заданному в [**файле шаблона строки**](/ru/assets/output.rows):
...
Результаты, сформированные по шаблону, также можно экспортировать в файлы с помощью конструкции [`INTO OUTFILE`](/ru/reference/statements/select/into-outfile). Давайте сгенерируем HTML-файлы на основе заданных форматов [resultset](/ru/assets/html.results) и [row](/ru/assets/html.row):
...
Формат [Regexp](/ru/reference/formats/Regexp) подходит для более сложных случаев, когда входные данные нужно разбирать более нетривиальным способом. Разберём наш файл [error.log](/ru/assets/error.log), но на этот раз извлечём имя файла и протокол, чтобы сохранить их в отдельные столбцы. Сначала подготовим для этого новую таблицу:

...
Запрос [легковесного `DELETE`](/snippets/delete) можно выполнить как легковесный `UPDATE`, а не как мутацию `ALTER UPDATE`. Реализация легковесного `DELETE` управляется настройкой [`lightweight_delete_mode`](/ru/reference/settings/session-settings#lightweight_delete_mode).
...
* [Легковесный `DELETE`](/snippets/delete) — операции легковесного `DELETE`

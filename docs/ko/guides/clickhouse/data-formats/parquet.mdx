...
데이터를 로드하기 전에 [file()](/ko/reference/functions/regular-functions/files#file) 함수를 사용해 [예시 Parquet 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.parquet)의 구조를 확인할 수 있습니다:
...
ClickHouse와 Parquet의 데이터 타입은 대부분 동일하지만, [약간의 차이](/ko/reference/formats/Parquet/Parquet#data-types-matching-parquet)가 있습니다. 예를 들어 ClickHouse는 `DateTime` 타입을 Parquet의 `int64`로 내보냅니다. 그런 다음 이를 다시 ClickHouse로 가져오면 숫자로 표시됩니다([time.parquet file](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/time.parquet)):
...
SELECT
    n,
    toDateTime(time)                 <--- int를 time으로 변환

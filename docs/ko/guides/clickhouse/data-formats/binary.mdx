...
예시를 위해 some&#95;data [테이블](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.sql)과 [데이터](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.tsv)를 사용하겠습니다. 사용 중인 ClickHouse 인스턴스에서 그대로 따라 해 보셔도 됩니다.
...
그러면 [data.clickhouse](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.clickhouse) 파일이 네이티브 형식으로 생성됩니다.
...
이렇게 하면 [data.binary](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.binary) 파일이 바이너리 행 형식으로 생성됩니다.
...
[MessagePack 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.msgpk)에서 데이터를 가져오려면:
...
[Protocol Buffers](/ko/reference/formats/Protobuf/Protobuf)를 사용하려면 먼저 [스키마 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.proto)을 정의해야 합니다.
...
이렇게 하면 데이터가 [proto.bin](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/proto.bin) 파일에 저장됩니다. ClickHouse는 Protobuf 데이터 가져오기와 중첩된 메시지도 지원합니다. 단일 Protocol Buffer 메시지를 처리하려면 [ProtobufSingle](/ko/reference/formats/Protobuf/ProtobufSingle) 사용을 고려하십시오(이 경우 길이 구분 기호는 생략됩니다).
...
ClickHouse가 지원하는 또 다른 널리 쓰이는 바이너리 형식의 직렬화 포맷은 [Cap&#39;n Proto](https://capnproto.org/)입니다. `Protobuf` format과 마찬가지로, 이 예시에서는 스키마 파일([`schema.capnp`](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.capnp))을 정의해야 합니다:

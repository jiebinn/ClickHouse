...
가장 널리 사용되는 JSON 데이터 형식 중 하나는 [이 예시](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)와 같이 JSON 배열에 JSON 객체 목록이 들어 있는 형태입니다:
...
JSON 객체 목록을 가져오려면 [`JSONEachRow`](/ko/reference/formats/JSON/JSONEachRow) 포맷을 사용할 수 있습니다([list.json](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) 파일에서 데이터를 삽입):
...
경우에 따라 JSON 객체 목록은 배열 요소가 아니라 객체 속성으로 인코딩될 수 있습니다([objects.json](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json)을 예시로 참조하십시오):
...
공간을 절약하기 위해 JSON 파일을 객체 대신 배열로 인코딩하는 경우가 있습니다. 이 경우 [JSON 배열 목록](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.jsonl)을 사용합니다:
...
경우에 따라 데이터는 행 단위가 아니라 컬럼 단위로 인코딩될 수 있습니다. 이 경우 상위 JSON 객체는 값을 담은 컬럼을 포함합니다. [다음 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json)을 확인하십시오:
...
객체 대신 [컬럼 배열](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json)을 다룰 때 사용할 수 있는 더 간결한 [`JSONCompactColumns`](/ko/reference/formats/JSON/JSONCompactColumns) 포맷도 지원됩니다:
...
JSON 객체를 파싱하지 않고 하나의 `String`(또는 `JSON`) 컬럼에 저장하는 편이 나은 경우도 있습니다. 이는 구조가 서로 다른 JSON 객체 목록을 다룰 때 유용합니다. 예를 들어 [이 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json)에는 상위 목록 안에 서로 다른 JSON 객체가 여러 개 들어 있습니다:
...
[중첩된 JSON 객체](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)를 다룰 때는 명시적인 스키마를 추가로 정의하고, 복합 타입([`Array`](/ko/reference/data-types/array), [`JSON`](/ko/guides/clickhouse/data-formats/json/intro) 또는 [`Tuple`](/ko/reference/data-types/tuple))을 사용해 데이터를 로드할 수 있습니다:
...
[다음 설정 옵션](/ko/reference/settings/formats#input_format_import_nested_json)을 활성화하면 [중첩된 JSON 키](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)를 참조할 수 있습니다:
...
3개의 컬럼으로 이루어진 [원본 JSON 데이터](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)를 이 테이블에 계속 삽입할 수 있습니다:
...
BSON 데이터를 가져오려면 [BSONEachRow](/ko/reference/formats/BSONEachRow) 포맷을 사용합니다. [이 BSON 파일](/kohttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson)에서 데이터를 가져와 보겠습니다:

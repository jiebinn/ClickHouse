...
[경량 `DELETE`](/snippets/delete) 쿼리는 `ALTER UPDATE` mutation 대신 경량 `UPDATE`로 실행할 수 있습니다. 경량 `DELETE`의 동작 방식은 [`lightweight_delete_mode`](/ko/reference/settings/session-settings#lightweight_delete_mode) 설정으로 제어됩니다.
...
* [Lightweight `DELETE`](/snippets/delete) - 경량 `DELETE` 작업

...
<Note>
For for services already exceeding the above usage limits, table/database limits are set at the service's current table/database count plus 25%.
</Note>

Your service's specific warn and throw limits can be verifyied by querying system.server_settings:
```
SELECT * 

...
Contains information about tasks in background schedule pools. Background schedule pools are used for executing periodic tasks such as distributed sends, buffer flushes, message broker operations, and streaming queries background jobs.

## Columns {#columns}

<!--AUTOGENERATED_START-->
- `pool` ([String](/sql-reference/data-types/string)) — Pool name. Possible values: `schedule` (general purpose schedule pool), `buffer_flush` (pool for flushing Buffer table data), `distributed` (pool for distributed table operations), `message_broker` (pool for message broker operations), `streaming` (pool for streaming queries background jobs).

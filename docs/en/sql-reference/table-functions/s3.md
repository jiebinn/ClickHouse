...
| `storage_class_name`                    | Parameter is optional. Supported values: `STANDARD`, `REDUCED_REDUNDANCY`, `STANDARD_IA`, `INTELLIGENT_TIERING`. Only S3 storage classes that allow immediate retrieval are supported (archival classes such as `GLACIER` are not). Allows to specify [AWS S3 Intelligent Tiering](https://aws.amazon.com/s3/storage-classes/intelligent-tiering/). Defaults to `STANDARD`. |
...
Role-based access for S3 in ClickHouse Cloud is documented [here](/cloud/data-sources/secure-s3).

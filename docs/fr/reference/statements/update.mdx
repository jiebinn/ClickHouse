...
Une requête [de `DELETE` léger](/snippets/delete) peut être exécutée sous la forme d’un `UPDATE` léger plutôt que d’une mutation `ALTER UPDATE`. Le fonctionnement de `DELETE` léger est contrôlé par le paramètre [`lightweight_delete_mode`](/fr/reference/settings/session-settings#lightweight_delete_mode).
...
* [Lightweight `DELETE`](/snippets/delete) - Opérations `DELETE` légères

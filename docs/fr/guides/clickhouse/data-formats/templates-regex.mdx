...
Supposons que nous voulions importer des données depuis le [fichier journal](/fr/assets/error.log) :
...
Pour importer des données à l’aide d’un modèle spécifique, nous devons enregistrer notre chaîne de modèle dans un fichier ([row.template](/fr/assets/row.template) dans notre cas) :
...
[Modèle du jeu de résultats](/fr/assets/output.results), qui définit la structure de l’ensemble du jeu de résultats :
...
Ici, `rows_read` et `time` sont des métriques système disponibles pour chaque requête. Quant à `data`, il correspond aux lignes générées (`${data}` doit toujours être le premier espace réservé dans ce fichier), à partir d’un modèle défini dans un [**fichier de modèle de ligne**](/fr/assets/output.rows) :
...
Les résultats basés sur des modèles peuvent également être exportés dans des fichiers à l’aide d’une clause [`INTO OUTFILE`](/fr/reference/statements/select/into-outfile). Générons des fichiers HTML à partir des formats [jeu de résultats](/fr/assets/html.results) et [ligne](/fr/assets/html.row) fournis :
...
Le format [Regexp](/fr/reference/formats/Regexp) permet de gérer des cas plus complexes, lorsque les données d’entrée doivent être analysées de manière plus élaborée. Analysons notre fichier d’exemple [error.log](/fr/assets/error.log), mais cette fois en capturant le nom du fichier et le protocole afin de les enregistrer dans des colonnes distinctes. Commençons par préparer une nouvelle table :

...
Pour importer à partir d’un [fichier Avro](/fr/assets/data.avro), il faut utiliser le format [Avro](/fr/reference/formats/Avro/Avro) dans l’instruction `INSERT` :
...
Un autre format colonnaire est [Apache Arrow](https://arrow.apache.org/), également pris en charge par ClickHouse pour l’importation et l’exportation. Pour importer des données à partir d’un [fichier Arrow](/fr/assets/data.arrow), nous utilisons le format [Arrow](/fr/reference/formats/Arrow/Arrow) :
...
Le [format Apache ORC](https://orc.apache.org/) est un format de stockage en colonnes généralement utilisé avec Hadoop. ClickHouse prend en charge l’importation comme l’exportation de [données ORC](/fr/assets/data.orc) à l’aide du [format ORC](/fr/reference/formats/ORC) :

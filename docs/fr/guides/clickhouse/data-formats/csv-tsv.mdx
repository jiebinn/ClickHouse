...
Pour importer des données depuis le [fichier CSV](/fr/assets/data_small.csv) vers la table `sometable`, nous pouvons rediriger notre fichier directement vers clickhouse-client :
...
Supposons que notre [fichier CSV comporte des en-têtes](/fr/assets/data_small_headers.csv) :
...
Le [fichier](/fr/assets/data_small.csv) contient 1k lignes, mais ClickHouse n’en a chargé que 990, car nous avons demandé à ignorer les 10 premières.
...
Le format de données séparées par des tabulations est couramment utilisé comme format d’échange de données. Pour charger des données depuis un [fichier TSV](/fr/assets/data_small.tsv) dans ClickHouse, on utilise le format [TabSeparated](/fr/reference/formats/TabSeparated/TabSeparated) :
...
Ce format inclura deux lignes d’en-tête : l’une avec les noms de colonnes et l’autre avec les types de colonnes. Cela permettra à ClickHouse (et à d’autres applications) d’identifier les types de colonnes lors du chargement de données depuis [ces fichiers](/fr/assets/data_csv_types.csv) :
...
Nous pouvons maintenant charger des données à partir de notre [fichier](/fr/assets/data_small_custom.txt) au format personnalisé :

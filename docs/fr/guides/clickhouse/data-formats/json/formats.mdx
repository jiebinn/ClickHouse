...
L’une des formes les plus courantes de données JSON est une liste d’objets JSON dans un tableau JSON, comme dans [cet exemple](/fr/assets/list.json) :
...
Pour importer une liste d’objets JSON, nous pouvons utiliser le format [`JSONEachRow`](/fr/reference/formats/JSON/JSONEachRow) (en insérant les données à partir du fichier [list.json](/fr/assets/list.json)) :
...
Dans certains cas, la liste d’objets JSON peut être encodée sous forme de propriétés d’objet plutôt que comme éléments de tableau (voir par exemple [objects.json](/fr/assets/objects.json)) :
...
Parfois, pour gagner de l&#39;espace, les fichiers JSON sont encodés sous forme de tableaux plutôt que d&#39;objets. Dans ce cas, il s&#39;agit d&#39;une [liste de tableaux JSON](/fr/assets/arrays.jsonl) :
...
Dans certains cas, les données peuvent être encodées en colonnes plutôt qu’en lignes. Dans ce cas, un objet JSON parent contient des colonnes avec leurs valeurs. Consultez le [fichier suivant](/fr/assets/columns.json) :
...
Un format plus compact est également pris en charge lorsqu&#39;on manipule un [tableau de colonnes](/fr/assets/columns-array.json) plutôt qu&#39;un objet, avec le format [`JSONCompactColumns`](/fr/reference/formats/JSON/JSONCompactColumns) :
...
Dans certains cas, vous souhaiterez peut-être stocker des objets JSON dans une seule colonne `String` (ou `JSON`) plutôt que de les analyser. Cela peut être utile lorsque vous traitez une liste d’objets JSON aux structures différentes. Prenons [ce fichier](/fr/assets/custom.json) comme exemple, où plusieurs objets JSON différents sont contenus dans une liste racine :
...
Lorsque nous traitons des [objets JSON imbriqués](/fr/assets/list-nested.json), nous pouvons également définir un schéma explicite et utiliser des types complexes ([`Array`](/fr/reference/data-types/array), [`JSON`](/fr/guides/clickhouse/data-formats/json/intro) ou [`Tuple`](/fr/reference/data-types/tuple)) pour charger les données :
...
Nous pouvons faire référence aux [clés JSON imbriquées](/fr/assets/list-nested.json) en activant [l’option de réglage suivante](/fr/reference/settings/formats#input_format_import_nested_json) :
...
Il est toujours possible d’insérer les [données JSON d’origine](/fr/assets/list.json) avec 3 colonnes dans cette table :
...
Pour importer des données BSON, nous utilisons le format [BSONEachRow](/fr/reference/formats/BSONEachRow). Importons des données à partir de [ce fichier BSON](/fr/assets/data.bson) :

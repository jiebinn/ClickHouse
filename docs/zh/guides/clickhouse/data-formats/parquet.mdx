...
在加载数据之前，我们可以使用 [file()](/zh/reference/functions/regular-functions/files#file) 函数查看[示例 Parquet 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.parquet)的结构：
...
ClickHouse 和 Parquet 的数据类型大多相同，但仍[略有差异](/zh/reference/formats/Parquet/Parquet#data-types-matching-parquet)。例如，ClickHouse 会将 `DateTime` 类型导出为 Parquet 的 `int64`。如果再将其导入回 ClickHouse，我们看到的将是数字 ([time.parquet file](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/time.parquet)) ：

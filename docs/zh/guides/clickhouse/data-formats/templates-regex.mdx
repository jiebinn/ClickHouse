...
假设我们要从以下[日志文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/error.log)导入数据：
...
要使用指定模板导入数据，我们需要先将模板字符串保存到文件中 (本例中为 [row.template](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/row.template)) ：
...
[结果集模板](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.results)，它定义了整个结果集的布局：
...
这里，`rows_read` 和 `time` 是每个请求都可用的系统指标。`data` 表示根据 [**行模板文件**](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.rows) 中定义的模板生成的行 (在此文件中，`${data}` 必须始终作为第一个占位符出现) ：
...
基于 Template 的结果也可以通过 [`INTO OUTFILE`](/zh/reference/statements/select/into-outfile) 子句导出到文件中。下面让我们基于给定的 [resultset](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/html.results) 和 [行](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/html.row) 格式生成 HTML 文件：
...
[Regexp](/zh/reference/formats/Regexp) 格式适用于更复杂的场景，即需要以更复杂的方式解析输入数据。下面继续解析示例文件 [error.log](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/error.log)，但这次会捕获文件名和协议，并将它们分别保存到单独的列中。首先，为此准备一个新表：

...
要将 [CSV 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_small.csv) 中的数据导入 `sometable` 表，可以直接通过管道将该文件传给 clickhouse-client：
...
假设我们的 [CSV 文件带有表头](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_small_headers.csv)：
...
该[file](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_small.csv)有 1k 行，但由于我们要求跳过前 10 行，因此 ClickHouse 只加载了 990 行。
...
制表符分隔格式广泛用作数据交换格式。要将 [TSV 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_small.tsv) 中的数据加载到 ClickHouse，请使用 [TabSeparated](/zh/reference/formats/TabSeparated/TabSeparated) 格式：
...
这种格式会包含两行表头——一行是列名，另一行是列类型。这样 ClickHouse (以及其他应用) 在从[此类文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_csv_types.csv)加载数据时，就能识别列类型：
...
现在我们可以从这个自定义格式的[file](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data_small_custom.txt)中加载数据：

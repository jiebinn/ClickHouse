...
JSON 数据最常见的一种形式，是在 JSON 数组中包含一组 JSON 对象，如[此示例](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)所示：
...
要导入一组 JSON 对象，可以使用 [`JSONEachRow`](/zh/reference/formats/JSON/JSONEachRow) 格式 (从 [list.json](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) 文件中插入数据) ：
...
在某些情况下，JSON 对象列表也可以编码为对象属性，而不是数组元素 (示例见 [objects.json](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json)) ：
...
有时，为了节省空间，JSON 文件会采用数组而不是对象的编码方式。在这种情况下，我们处理的是一份 [JSON 数组列表](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.jsonl)：
...
在某些情况下，数据可以按列而非按行编码。在这种情况下，外层 JSON 对象包含各列及其值。请查看[以下文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json)：
...
如果处理的是[列数组](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json)而不是对象，也支持使用更紧凑的 [`JSONCompactColumns`](/zh/reference/formats/JSON/JSONCompactColumns) 格式：
...
在某些情况下，你可能希望将 JSON 对象保存到单个 `String` (或 `JSON`) 列中，而不是对其进行解析。在处理由不同结构的 JSON 对象组成的列表时，这会很有用。以[这个文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json)为例，其中父列表内包含多个不同的 JSON 对象：
...
在处理[嵌套 JSON 对象](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)时，我们还可以显式定义 schema，并使用复杂类型 ([`Array`](/zh/reference/data-types/array)、[`JSON`](/zh/guides/clickhouse/data-formats/json/intro) 或 [`Tuple`](/zh/reference/data-types/tuple)) 来加载数据：
...
启用[以下设置选项](/zh/reference/settings/formats#input_format_import_nested_json)后，我们就可以引用[嵌套 JSON 键](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)：
...
我们仍然可以将包含 3 列的[原始 JSON 数据](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)插入这个表中：
...
要导入 BSON 数据，我们使用 [BSONEachRow](/zh/reference/formats/BSONEachRow) 格式。下面从[这个 BSON 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson)导入数据：

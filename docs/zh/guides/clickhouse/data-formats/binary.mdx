...
我们将使用 some&#95;data [表](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.sql) 和 [数据](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/some_data.tsv) 进行演示，你也可以在自己的 ClickHouse 实例上复现这些示例。
...
这会以 Native 格式创建 [data.clickhouse](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.clickhouse) 文件。
...
这会生成一个 [data.binary](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.binary) 文件，其格式为二进制行格式。
...
要从 [MessagePack 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.msgpk) 中导入数据：
...
要使用 [Protocol Buffers](/zh/reference/formats/Protobuf/Protobuf)，首先需要定义一个 [schema 文件](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.proto)：
...
这会将数据保存到 [proto.bin](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/proto.bin) 文件。ClickHouse 还支持导入 Protobuf 数据以及嵌套消息。若要处理单个 Protocol Buffer 消息，可考虑使用 [ProtobufSingle](/zh/reference/formats/Protobuf/ProtobufSingle) (这种情况下会省略长度分隔符) 。
...
ClickHouse 支持的另一种常见二进制序列化格式是 [Cap&#39;n Proto](https://capnproto.org/)。与 `Protobuf` 格式类似，在本示例中，我们也需要定义一个 schema 文件 ([`schema.capnp`](/zhhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/schema.capnp)) ：

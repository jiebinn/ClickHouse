...
All ClickPipes types can be created, updated, and deleted programmatically using the [ClickHouse Cloud OpenAPI](/products/cloud/api-reference/organization/get-list-of-available-organizations#tag/ClickPipes). This page covers authentication and the available ClickPipes endpoints, with example `curl` requests for each ClickPipe type.
...
For the full request and response schemas for each endpoint, see the [Swagger UI](/products/cloud/api-reference/organization/get-list-of-available-organizations#tag/ClickPipes).
...
The `POST /clickpipes` request body varies by source type. The examples below show the structure for each supported ClickPipe type. For the authoritative JSON schemas, refer to the [Swagger UI](/products/cloud/api-reference/organization/get-list-of-available-organizations#tag/ClickPipes).

...
A Postgres ClickPipe can also be created and managed via [OpenAPI](/products/cloud/features/admin-features/api/openapi) endpoints. This feature is in beta, and the API reference can be found [here](/products/cloud/api-reference/organization/get-list-of-available-organizations#tag/beta). We're actively working on Terraform support to create Postgres ClickPipes as well.

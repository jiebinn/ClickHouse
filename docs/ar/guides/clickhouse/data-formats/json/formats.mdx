...
من أكثر أشكال بيانات JSON شيوعًا أن تكون هناك قائمة من كائنات JSON ضمن مصفوفة JSON، كما في [هذا المثال](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json):
...
لاستيراد قائمة من كائنات JSON، يمكن استخدام تنسيق [`JSONEachRow`](/ar/reference/formats/JSON/JSONEachRow) (لإدراج البيانات من ملف [list.json](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)):
...
في بعض الحالات، يمكن ترميز قائمة من كائنات JSON كخصائص للكائن بدلًا من عناصر في المصفوفة (راجع [objects.json](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json) للاطلاع على مثال):
...
أحيانًا، لتوفير المساحة، تُرمَّز ملفات JSON على هيئة مصفوفات بدلًا من كائنات. في هذه الحالة، نتعامل مع [قائمة من مصفوفات JSON](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.jsonl):
...
في بعض الحالات، يمكن ترميز البيانات على مستوى الأعمدة بدلًا من الصفوف. في هذه الحالة، يحتوي كائن JSON رئيسي على أعمدة تتضمن قيمًا. ألقِ نظرة على [الملف التالي](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json):
...
كما يُدعَم أيضًا تنسيق أكثر إحكامًا عند التعامل مع [مصفوفة من الأعمدة](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json) بدلًا من كائن، باستخدام تنسيق [`JSONCompactColumns`](/ar/reference/formats/JSON/JSONCompactColumns):
...
قد تكون هناك حالات ترغب فيها في حفظ كائنات JSON في عمود `String` واحد (أو `JSON`) بدلًا من تحليلها. وقد يكون ذلك مفيدًا عند التعامل مع قائمة من كائنات JSON ذات بُنى مختلفة. لنأخذ [هذا الملف](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json) على سبيل المثال، حيث لدينا عدة كائنات JSON مختلفة داخل قائمة رئيسية:
...
في الحالات التي نتعامل فيها مع [كائنات JSON المتداخلة](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)، يمكننا أيضًا تحديد مخطط صريح واستخدام الأنواع المعقدة ([`Array`](/ar/reference/data-types/array)، [`JSON`](/ar/guides/clickhouse/data-formats/json/intro) أو [`Tuple`](/ar/reference/data-types/tuple)) لتحميل البيانات:
...
يمكن الإشارة إلى [مفاتيح JSON المتداخلة](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json) عبر تفعيل [خيار الإعدادات التالي](/ar/reference/settings/formats#input_format_import_nested_json):
...
لا يزال بإمكاننا إدراج [بيانات JSON الأصلية](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) ذات الأعمدة الثلاثة في هذا الجدول:
...
لاستيراد بيانات BSON، نستخدم تنسيق [BSONEachRow](/ar/reference/formats/BSONEachRow). لنستورد البيانات من [ملف BSON هذا](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson):

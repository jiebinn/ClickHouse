...
لنفترض أننا نريد استيراد البيانات من [ملف السجل](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/error.log) التالي:
...
يمكننا استخدام تنسيق [Template](/ar/reference/formats/Template/Template) لاستيراد هذه البيانات. ويجب تعريف سلسلة قالب تتضمن عناصر نائبة للقيم لكل صف من بيانات الإدخال:
...
لاستيراد البيانات باستخدام قالب معيّن، علينا حفظ سلسلة القالب الخاصة بنا في ملف ([row.template](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/row.template) في حالتنا):
...
[قالب مجموعة النتائج](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.results)، الذي يحدّد بنية مجموعة النتائج بالكامل:
...
هنا، يُمثّل `rows_read` و`time` مقاييس النظام المتاحة لكل طلب. ويشير `data` إلى الصفوف المُولَّدة (يجب أن يأتي `${data}` دائمًا كأول عنصر نائب في هذا الملف)، استنادًا إلى قالب مُعرَّف في [**ملف قالب الصفوف**](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/output.rows):
...
يمكن أيضًا تصدير النتائج المعتمدة على القوالب إلى ملفات باستخدام عبارة [`INTO OUTFILE`](/ar/reference/statements/select/into-outfile). لنُنشئ ملفات HTML استنادًا إلى تنسيقي [مجموعة النتائج](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/html.results) و[الصف](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/html.row) المحدَّدين:
...
يُستخدم تنسيق [Regexp](/ar/reference/formats/Regexp) للحالات الأكثر تعقيدًا، عندما يلزم parse بيانات الإدخال بطريقة أكثر تعقيدًا. لنحلّل ملف المثال [error.log](/arhttps://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/error.log)، لكن هذه المرة سنستخرج اسم الملف والبروتوكول لحفظهما في أعمدة منفصلة. أولًا، لنُعِد جدولًا جديدًا لهذا الغرض:

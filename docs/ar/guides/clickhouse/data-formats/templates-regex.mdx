...
لنفترض أننا نريد استيراد البيانات من [ملف السجل](/ar/assets/error.log) التالي:
...
لاستيراد البيانات باستخدام قالب معيّن، علينا حفظ سلسلة القالب الخاصة بنا في ملف ([row.template](/ar/assets/row.template) في حالتنا):
...
[قالب مجموعة النتائج](/ar/assets/output.results)، الذي يحدّد بنية مجموعة النتائج بالكامل:
...
هنا، يُمثّل `rows_read` و`time` مقاييس النظام المتاحة لكل طلب. ويشير `data` إلى الصفوف المُولَّدة (يجب أن يأتي `${data}` دائمًا كأول عنصر نائب في هذا الملف)، استنادًا إلى قالب مُعرَّف في [**ملف قالب الصفوف**](/ar/assets/output.rows):
...
يمكن أيضًا تصدير النتائج المعتمدة على القوالب إلى ملفات باستخدام عبارة [`INTO OUTFILE`](/ar/reference/statements/select/into-outfile). لنُنشئ ملفات HTML استنادًا إلى تنسيقي [مجموعة النتائج](/ar/assets/html.results) و[الصف](/ar/assets/html.row) المحدَّدين:
...
يُستخدم تنسيق [Regexp](/ar/reference/formats/Regexp) للحالات الأكثر تعقيدًا، عندما يلزم parse بيانات الإدخال بطريقة أكثر تعقيدًا. لنحلّل ملف المثال [error.log](/ar/assets/error.log)، لكن هذه المرة سنستخرج اسم الملف والبروتوكول لحفظهما في أعمدة منفصلة. أولًا، لنُعِد جدولًا جديدًا لهذا الغرض:

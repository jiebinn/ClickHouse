...
يمكن تنفيذ استعلام [lightweight `DELETE`](/snippets/delete) على شكل `UPDATE` خفيف الوزن بدلًا من mutation `ALTER UPDATE`. ويُتحكَّم في تنفيذ lightweight `DELETE` من خلال الإعداد [`lightweight_delete_mode`](/ar/reference/settings/session-settings#lightweight_delete_mode).
...
* [Lightweight `DELETE`](/snippets/delete) - عمليات `DELETE` خفيفة الوزن

...
Uma consulta de [exclusão leve](/snippets/delete) pode ser executada como um `UPDATE` leve em vez de uma mutação `ALTER UPDATE`. A implementação de exclusão leve é controlada pela configuração [`lightweight_delete_mode`](/pt-BR/reference/settings/session-settings#lightweight_delete_mode).
...
* [exclusão leve](/snippets/delete) - Operações de exclusão leve

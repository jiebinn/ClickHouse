...
def build_quickstarts(quickstarts_dir: Path, project_root: Path,
                      update_badges: bool) -> Tuple[List[Dict[str, Any]], int]:
...
def main():
    ...
    for locale in locales:
        ...
        locale_quickstarts, locale_failures = build_quickstarts(
            locale_dir, project_root, update_badges=False)
    ...
    if failures:
        print(f"\n✗ {failures} page(s) failed; the generated data omits them "
              "or their badges are stale — fix the errors above and re-run.")
        return 1
    return 0

...
- `galaxy.js` — initializes ClickHouse Galaxy analytics, tracks docs and knowledge-base page lifecycle events across Mintlify SPA navigation, batches events to the Galaxy API (disabled on localhost), and carries attribution plus the Galaxy visitor ID into ClickHouse Cloud links.
...
Run the Galaxy legacy-contract regression test with:

```bash
node --test _site/customizations/galaxy.test.cjs
```

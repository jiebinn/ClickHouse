...
void Timer::start(UInt32 period)
{
    ...
    if (timer_settime(*timer_id, 0, &timer_spec, nullptr))
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Failed to set thread timer period");
}
...
void Timer::tryStop()
{
    if (timer_id.has_value())
    {
        ...
        if (err)
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Failed to stop query profiler timer");
...
void Timer::tryCleanup()
{
    if (timer_id.has_value())
    {
        ...
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Failed to delete query profiler timer");
...
thread_local Timer QueryProfilerBase<ProfilerImpl>::timer = Timer();
#endif

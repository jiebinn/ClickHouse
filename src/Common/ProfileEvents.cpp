...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(SparseOffsetsShareSeedHits, "Number of scan-side attempts, one per wide-part `readRows` call and storage column, where the per-query `SparseOffsetsShare` had analyzer-produced offsets covering the requested row window and seeded that call's `SubstreamsCache`.", ValueType::Number) \
    M(SparseOffsetsShareSeedMisses, "Number of scan-side attempts, one per wide-part `readRows` call and storage column, where a per-query `SparseOffsetsShare` was attached but had no `(part, column)` bucket or no stored range covering the requested row window, so the `SparseOffsets` substream had to be read normally.", ValueType::Number) \
    M(SparseOffsetsShareConsumed, "Number of times `SerializationSparse` consumed a `SubstreamsCache` element seeded from `SparseOffsetsShare` instead of reading `SparseOffsets`. This cache element is per read call; `SparseOffsetsShare` itself is per query.", ValueType::Number) \

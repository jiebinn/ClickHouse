...
{
    std::vector<char> buffer;       /// The contiguous .pst-like data buffer
    TokenPostingsInfo info;         /// Populated TokenPostingsInfo
    std::vector<uint32_t> all_docs; /// All doc IDs in order (for verification)

    /// Resources kept alive for cursor (populated by makeMultiBlockCursor).
    mutable std::shared_ptr<DiskLocal> disk;
...
MultiBlockTestData makeMultiBlockData(const std::vector<std::vector<uint32_t>> & blocks)
{
    ...
    auto str = out.str();
    result.buffer.assign(str.begin(), str.end());
    return result;
}

/// Helper: create a PostingListCursor for multi-block test data.
/// Writes the binary buffer to a temporary .pst file, constructs a real
/// MergeTreeReaderStreamSingleColumn via DiskLocal + DataPartStorageOnDiskFull,
/// and uses the production PostingListCursor constructor.
PostingListCursorPtr makeMultiBlockCursor(const MultiBlockTestData & data)
{
    ...
    /// Construct the stream infrastructure — resources stored in `data` (mutable)
    /// to keep them alive as long as the test holds `data`.
    data.disk = std::make_shared<DiskLocal>("test_disk", tmp_dir.string() + "/");
    data.volume = std::make_shared<SingleDiskVolume>("test_vol", data.disk);
    data.storage_holder = std::make_shared<DataPartStorageOnDiskFull>(data.volume, "", "part");
    ...
    data.stream = std::make_unique<MergeTreeReaderStreamSingleColumnWholePart>(
        data.storage_holder,
        "stream",
        ".pst",
        marks_count,
...
TEST(PostingListCursorTest, MultiBlockLinearAndFullRange)
{
    ...
    /// Check some docs in the gap remain 1 (not incremented)
    EXPECT_EQ(buf[200], 1u);
    EXPECT_EQ(buf[400], 1u);
}
...
TEST(PostingListCursorTest, ArithmeticZeroDeltaLinearAnd)
{
    ...
    /// Positions beyond the posting list remain at 1 (not incremented).

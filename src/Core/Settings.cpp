...
Optimize multiple `OR LIKE/ILIKE/match` into `multiSearchAny` (for substring patterns), `multiMatchAny` (for other patterns when Hyperscan/Vectorscan is permitted), or `match` with a combined regexp (otherwise). With the new analyzer, for pure `LIKE`/`ILIKE`/`match` `OR` chains the original expressions are preserved in `indexHint()` to allow index analysis; mixed `OR` chains that include non-`LIKE` branches intentionally skip `indexHint()` wrapping so that ranges matching only the non-`LIKE` branch are not pruned. The old analyzer (`enable_analyzer = 0`) does not preserve `indexHint()` at all, so MergeTree index analysis cannot use the original `LIKE` chain after the rewrite. The choice between `multiMatchAny` and `match` honors `allow_hyperscan`, `max_hyperscan_regexp_length`, `max_hyperscan_regexp_total_length` and `reject_expensive_hyperscan_regexps`: the rewrite picks `multiMatchAny` only when those settings allow it and falls back to `match` with a combined regexp otherwise. Only chains with at least `optimize_or_like_chain_min_patterns` branches sharing the same left-hand-side expression are rewritten; shorter chains are kept as-is to avoid regressing queries where the fixed setup cost of `multiSearchAny`/Hyperscan dominates over evaluating the original `OR` chain.
)", 0) \
    DECLARE(UInt64, optimize_or_like_chain_min_patterns, 5, R"(
Minimum number of LIKE/ILIKE/match branches that share the same left-hand-side expression required for optimize_or_like_chain to rewrite them. Calibrated against performance/optimize_or_like_chain_hits.xml: chains of four or fewer patterns typically regress (the fixed setup cost of multiSearchAny / multiMatchAny outweighs short-circuit evaluation of a few LIKE branches), while chains of eleven or more patterns improve substantially. A value of 0 or 1 disables the threshold and rewrites every chain. Has no effect when optimize_or_like_chain is disabled.
)", 0) \

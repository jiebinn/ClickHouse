...
For `BACKUP`/`RESTORE ... ON CLUSTER` the restriction is enforced on the initiator only. The per-host continuation queries it spawns (marked `internal`, which a user cannot set) do not re-check it: they run through the distributed DDL queue, by default without the initiator's user, so re-checking would apply the default profile instead of the settings of the user who was already allowed to open the same backup destination on the initiator.

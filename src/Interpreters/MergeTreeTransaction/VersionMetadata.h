...
public:
    ...
    /// Temporary file written before the atomic rename to `TXN_VERSION_METADATA_FILE_NAME`. If only this file is
    /// present (no final file), the creating transaction never committed, so the part must be treated as rolled back.

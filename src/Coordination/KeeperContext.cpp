...
{
    uint64_t version;
    CoordinationSettings * ptr;
};

/// State for thread-local caching of CoordinationSettings struct.
/// Separate thread-locals for POD and RAII parts, because there's a tiny bit of overhead when
/// accessing non-trivially-initialized thread-locals (because of the lazy initialization check
/// generated by the compiler).
thread_local CachedCoordinationSettings cached_coordination_settings = {0, nullptr};
thread_local CoordinationSettingsPtr cached_coordination_settings_holder;
/// Assign unique versions across all instances of KeeperContext.
static std::atomic<uint64_t> next_coordination_settings_version {1};

KeeperContext::KeeperContext(bool standalone_keeper_, CoordinationSettingsPtr coordination_settings)
    : disk_selector(std::make_shared<DiskSelector>())
    , standalone_keeper(standalone_keeper_)
{
    coordination_settings->version = next_coordination_settings_version++;
    fixed_settings = coordination_settings;
    dynamic_settings = coordination_settings;
...
const CoordinationSettings & KeeperContext::getCoordinationSettings() const
{
    ...
    /// happen, except maybe in tests), the thread-local caching won't work well, and many calls
    /// will fall back to the slow path. That's ok as the slow path is not all that slow.
    auto & cached = cached_coordination_settings;
    uint64_t latest_version = settings_version.load(std::memory_order_acquire);
    if (cached.version == latest_version)
        return *cached.ptr;

    CoordinationSettingsPtr settings;
    {
        /// Slow path: increment shared refcount.
        std::shared_lock lock(settings_mutex);
        settings = dynamic_settings;
    }
    cached.version = settings->version;
    cached.ptr = settings.get();
    cached_coordination_settings_holder = std::move(settings); // decrement old version's refcount
    return *cached.ptr;
}

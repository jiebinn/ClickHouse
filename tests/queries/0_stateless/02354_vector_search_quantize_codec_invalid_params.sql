...
-- dimension. The `mrl` method takes Quantized('mrl', dimensions, leading_dimensions, 'int8'|'bf16').
...
CREATE TABLE quantize_bad (id UInt32, vec Array(Float32) CODEC(Quantized('mrl', 64, 32, 'fp8'))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }
-- The number of leading dimensions must be in [1, dimensions].
CREATE TABLE quantize_bad (id UInt32, vec Array(Float32) CODEC(Quantized('mrl', 64, 0, 'int8'))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }
CREATE TABLE quantize_bad (id UInt32, vec Array(Float32) CODEC(Quantized('mrl', 64, 128, 'int8'))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }
...
CREATE TABLE quantize_mrl_ok (id UInt32, vi Array(Float32) CODEC(Quantized('mrl', 64, 32, 'int8')), vb Array(Float32) CODEC(Quantized('mrl', 64, 32, 'bf16'))) ENGINE = MergeTree ORDER BY id;

...
-- cast to `ColumnArray`. Covers a data-independent method (`int8`) and the trained `pq` method (whose training also casts).
...
CREATE TABLE quantize_const_pq (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 8, 4, 2))) ENGINE = MergeTree ORDER BY id;
...
SELECT 'pq', count(), uniqExact(length(vec.quantized)) AS distinct_code_lengths FROM quantize_const_pq;

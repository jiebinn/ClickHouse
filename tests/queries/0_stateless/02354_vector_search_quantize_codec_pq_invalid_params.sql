...
-- The `pq` method of the `Quantized(...)` codec uses the syntax `Quantized('pq', dimensions, nbits, m)` and validates its
...
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_SYNTAX_FOR_CODEC_TYPE }
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 8))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_SYNTAX_FOR_CODEC_TYPE }

-- `dimensions` must be a multiple of `m` (64 is not a multiple of 7).
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 8, 7))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }

-- `m` must be greater than zero.
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 8, 0))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }

-- `nbits` must be in [1, 16] (it builds a 2^nbits codebook per subspace).
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 0, 8))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 17, 8))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }
...
CREATE TABLE quantize_pq_bad (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 16, 8))) ENGINE = MergeTree ORDER BY id; -- { serverError ILLEGAL_CODEC_PARAMETER }

-- A valid 4-argument `pq` codec is accepted and its code is m = 8 bytes (one byte per subspace at nbits = 8).
CREATE TABLE quantize_pq_ok (id UInt32, vec Array(Float32) CODEC(Quantized('pq', 64, 8, 8))) ENGINE = MergeTree ORDER BY id;

...
SELECT arraySort(materialize([toFloat32(1.), nan, 2., 3., nan, -4., inf, -inf]));
SELECT arrayReverseSort(materialize([toFloat32(1.), nan, 2., 3., nan, -4., inf, -inf]));
...
SELECT sum(toString(arraySort(a)) != toString(arrayMap(t -> t.1, arraySort(arrayMap(x -> tuple(x), a)))))
